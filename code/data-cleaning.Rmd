...
En primer lugar, vamos a comprobar la normalidad y homogeneidad de la varianza. Para ello, primero vamos a transformar las columnas categóricas, y analizaremos la normalidad de aquellas variables que no sean categóricas. Para revisar si las variables pueden ser candidatas a la normalización miramos las graficas de quantile-quantile plot y el histograma.
```{r ,eval=TRUE,echo=TRUE}
# Convertir las columnas a tipo categórico
columnas_categoricas <- c("sex", "exng", "caa", "cp", "fbs", "restecg", "thall", "slp", "output")
data[columnas_categoricas] <- lapply(data[columnas_categoricas], as.factor)
